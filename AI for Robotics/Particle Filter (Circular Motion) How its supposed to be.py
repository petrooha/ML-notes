...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if distance < 0.0:
            raise ValueError, 'Moving backwards is no valid'
        ...
        turn = tan(steering2) * distance2 / car.length # given formula

        if abs(turn) < 0.001:
            # approximate by straight line motion

            car.x = self.x + (distance2 * cos(self.orientation))
            car.y = self.y + (distance2 * sin(self.orientation))
            car.orientation = (self.orientation + turn) % (2.0 * pi)

            # approximate bicycle model for motion (given formulas)
        else:
            ...
            car.y = cy - (cos(car.orientation) * radius)

...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        for i in range(T):
            d = motions[i][1]
            if motions[i][0] < 0.01:
                x = x + d * cos(orientation)
                y = y + d * sin(orientation)
            else:
                betta = float(tan(motions[i][0]))*d/length
                
                R = d/betta
                cx = x - sin(orientation)*R
                cy = y + cos(orientation)*R

                x = cx + sin(orientation + betta) * R
                y = cy - cos(orientation + betta) * R
                orientation = (orientation + betta)
                #print x, y, orientation
            result.append([x, y, orientation])
        return result                       

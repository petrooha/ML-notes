...
#then:
total_lessons_by_acct = {}#-> {student : total lessons}
...
for account_key, list_of_engagements in eng_by_acct.items():# looping through {x : []}
    ...
    for record in list_of_engagements:# looping through each []
        total_lessons += float(record['lessons_completed'])
    total_lessons_by_acct[account_key] = total_lessons

total_lessons = total_lessons_by_acct.values()# to get list of values from dict

import numpy as np

#print 'Mean:', np.mean(total_lessons) # Solution!
...
passmin = []
nopassmin = []
for key in total_lessons_by_acct:
    if key in passing:
        passmin.append(total_lessons_by_acct[key])
    else:
        nopassmin.append(total_lessons_by_acct[key])
